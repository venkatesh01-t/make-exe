...
class ClinicManager(ctk.CTk if ctk else tk.Tk):
    ...
    def startup_checks(self):
        ...
        try:
            ...
            # Step 1: Download and extract if needed
            ...
            self.append_output('Step 1: Checking if clinic repo needs download...')
    ...
    def download_and_extract(self):
        ...
        # Check if clinic folder already exists
        if (WORKSPACE / 'clinic').exists():
            self.append_output('Clinic folder already present, skipping download')
            return
        ...
        try:
            ...
            if not extraction_success:
                ...
                try:
                    ...
                    self.extract_zip_alternative(dest_zip, WORKSPACE)
                except Exception as e:
    ...
    def check_folders(self):
        expected = ['clinic', '3.11.9']
        missing = [d for d in expected if not (WORKSPACE / d).exists()]
        if missing:
            self.append_output('Missing folders: ' + ', '.join(missing))
            messagebox.showwarning('Missing', 'Missing: ' + ', '.join(missing))
        else:
            self.append_output('All expected folders present')

    def install_requirements(self):
        """Install all packages from requirement.txt."""
        req = WORKSPACE / 'requirement.txt'
    ...
    def start_server(self):
        ...
        try:
            if sys.platform == 'win32':
                ...
                # Server runs in background with no visible window
                cmd = [str(PYTHON_EXE), str(PROJECT_DIR / 'manage.py'), 'runserver', f'0.0.0.0:{port}']
                
                # CREATE_NO_WINDOW hides console completely
    ...
    def open_browser(self):
        ...
        if not url or not self.server_running:
...
5. First run setup:
   - Automatically downloads clinic repository
   - Extracts files silently in background
   - Installs Python requirements in background
   - All happens while splash screen shows progress
